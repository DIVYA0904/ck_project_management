...
<body>
            ...
            <div class="modal fade" id="createmodel" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="post" action="javascript:void(0)" id="dept_form">
                            <div class="modal-header">
                                <h5 class="modal-title" id="createModalLabel"><i class="ti-marker-alt m-r-10"></i> Add  Department</h5>
                            ...
                            <div class="modal-body">
                                <div class="input-group mb-3">
                                    ...
                                    <input type="text" class="form-control" name="dept" id="dept" placeholder="Department" aria-label="name">
                                </div>
                                <span id="dept_error" class="text-danger"></span>
                            </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="post" action="javascript:void(0)" id="edit_form">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editmodelLabel"><i class="ti-marker-alt m-r-10"></i> Edit Department</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <div class="input-group mb-3">
                                    <button type="button" class="btn btn-info"><i class="ti-user text-white"></i></button>
                                    <input type="text" class="form-control" id="mytest" name="dept" placeholder="Department" aria-label="name">
                                </div>
                                <span id="dept_error_edit" class="text-danger"></span>                                     
                            </div>
                            <div class="modal-footer">
                                <input type="hidden" name="id" id="dept_id" class="form-control">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="post" action="javascript:void(0)" id="delete_form">
                            <div class="modal-header">
                                <h5 class="modal-title" id="deletemodelLabel"><i class="ti-marker-alt m-r-10"></i> Delete Department</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="post" action="javascript:void(0)" id="active_form">
                            <div class="modal-header">
                                <h5 class="modal-title" id="activemodelLabel"><i class="ti-marker-alt m-r-10"></i> Active Department</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="post" action="javascript:void(0)" id="deactive_form">
                            <div class="modal-header">
                                <h5 class="modal-title" id="activemodelLabel"><i class="ti-marker-alt m-r-10"></i> Deactive Department</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
    ...
    $("#save").on('click',()=>{
        ...
        // console.log(dept);
        $.ajax({
           url:"<?php echo base_url();?>/index.php/dept_insert",
           type:"POST",
           ...
           success:function(data)
           {
                if(data.error)
                {
                    if(data.dept_error != '')
                    {
                    $('#dept_error').html(data.dept_error);
                    }
                    else
                    {
                    $('#dept_error').html('');
                    ...
                    var dept_error = "The Department name field must contain a unique value.";                   
                    $('#dept_error').html(dept_error);                   
    ...
    <script type="text/javascript">
        ...
        function dept_data(){   
            ...
            var ct = $('#file_export').DataTable({ 
                ...
                'ajax': {
                    // type: 'POST',
                    'url': "<?= base_url(); ?>/index.php/dept_report_data",
                    // 'url': BASE_URL + 'welocme/index',
                    // 'url': BASE_URL + 'welocme/Department_report_data',
                    // "datetype": "json",
                    'data': function(data) {
                        // if(data){
                        //     alert(data)
                ...
                createdRow: function( row, data, dataIndex ) {
                    ...
                    $( row ).find('td:eq(1)').attr('data-label', 'Department');
        ...
        $("#edit").on('click',()=>{
            ...
            // alert(mytest);
            $.ajax({
                url:"<?php echo base_url();?>/index.php/dept_update",
                type:"POST",
                ...
                {            
                    if(data.error)
                    {
                        if(data.dept_error != '')
                        {
                        $('#dept_error_edit').html(data.dept_error);
                        }
                        else
                        {
                        $('#dept_error_edit').html('');
                        }
                    }
                    else
                    {
        ...
        $("#delete").on('click',()=>{
            ...
            $.ajax({
            url:"<?php echo base_url();?>/index.php/dept_delete",
            type:"POST",
        ...
        $("#active").on('click',()=>{
            ...
            $.ajax({
            url:"<?php echo base_url();?>/index.php/dept_active",
            type:"POST",
        ...
        $("#deactive").on('click',()=>{
            ...
            $.ajax({
            url:"<?php echo base_url();?>/index.php/dept_deactive",
            type:"POST",
